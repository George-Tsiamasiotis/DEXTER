...
use common::fallible_primitive_getter_impl;
...
impl Frequencies {
    ...
    fallible_primitive_getter_impl!(omega_theta, f64, "Retruns the calculated ωθ.");
    fallible_primitive_getter_impl!(omega_zeta, f64, "Retruns the calculated ωζ.");
    fallible_primitive_getter_impl!(qkinetic, f64, "Retruns the calculated q_kinetic.");

    /// Returns a tuple with the `(number of ψp intersections, step of each intersection)`.
    pub fn psip_intersections(&self) -> (usize, Vec<usize>) {
        self.psip_intersections.clone()
    }

    /// Returns a tuple with the `(number of θ intersections, step of each intersection)`.
...
pub(crate) fn close_theta_period(
    ...
    particle.evolution = Evolution::new();
    particle
        .initial_state
