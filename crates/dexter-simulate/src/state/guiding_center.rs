...
pub(crate) struct GCState {
    /// Keeps track of the coordinate with which the integration is performed.
...
impl GCState {
    ...
    /// Returns a reference to the [`MagneticFlux`] with respect to which the integration is
    /// performed.
    ...
    /// Returns a reference to the value of the [`MagneticFlux`] with respect to which the
    /// integration is performed.
    pub(crate) fn flux_value(&self) -> f64 {
        self.flux().value()
    }

    /// Returns a mutable reference to the value of the [`MagneticFlux`] with respect to which
    ...
    /// Returns a reference to the **other** [`MagneticFlux`], e.g. **not** the one with respect to
...
impl GCState {
    ...
    fn calculate_other_flux(
        ...
        let flux = *self.flux();
        match machine.qfactor().eval_other(flux, caches.flux_acc()) {
    ...
    fn calculate_qfactor_quantities(
        ...
        let flux = *self.flux();
        self.q = machine.qfactor().eval_q(flux, caches.flux_acc())?;
        Ok(())
    }
