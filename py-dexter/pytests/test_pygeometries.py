import pytest
from dexter import NcGeometry, _TOROIDAL_TEST_NETCDF_PATH, _POLOIDAL_TEST_NETCDF_PATH
...
def test_nc_geometry_getters(nc_geometry: NcGeometry):
    ...
    assert isinstance(nc_geometry.shape, tuple)
...
def test_nc_geometry_eval(nc_geometry: NcGeometry):
    ...
    psip = 0.015
    theta = 3.14
    assert isinstance(nc_geometry.psip_of_psi(psi), float)
    assert isinstance(nc_geometry.psi_of_psip(psip), float)
    assert isinstance(nc_geometry.r_of_psi(psi), float)
    assert isinstance(nc_geometry.r_of_psip(psip), float)
    assert isinstance(nc_geometry.psi_of_r(r), float)
    assert isinstance(nc_geometry.psip_of_r(r), float)
    assert isinstance(nc_geometry.rlab_of_psi(psi, theta), float)
    assert isinstance(nc_geometry.rlab_of_psip(psip, theta), float)
    assert isinstance(nc_geometry.zlab_of_psi(psi, theta), float)
    assert isinstance(nc_geometry.zlab_of_psip(psip, theta), float)
    assert isinstance(nc_geometry.jacobian_of_psi(psi, theta), float)
    assert isinstance(nc_geometry.jacobian_of_psip(psip, theta), float)
